...
def test_nmr_model(model_path, data_path, save_dir='results/testing/combined', 
    ...
    if WORKERS == 1:
        ...
        with tqdm(total=total_reconstructions, desc="Testing", unit="recon") as pbar:
            ...
            pbar.update(total_reconstructions)
    elif WORKERS > 1:
        worker_devices = _resolve_worker_devices(device, WORKERS)
        indices = list(range(n_samples))
        chunk_size = math.ceil(len(indices) / WORKERS)
        chunks = [indices[i*chunk_size:(i+1)*chunk_size] for i in range(WORKERS)]
        total_reconstructions = n_samples * mask_realizations_per_sample
...
if __name__ == "__main__":
    ...
    DATA_PATH = "data/tbi_tirupati/aligned_128K_TBI_Tirupati_WS625to680Zero.npy"
    SAVE_DIR = "results/testing/plasma"
    SUFFIX = "_50mask"  # Suffix for result files
    MASK_RATIO = 0.50 # Should match training
    ...
    df, summary = test_nmr_model(
        ...
        data_path=DATA_PATH,
        save_dir=SAVE_DIR,
        n_samples=None,  # None = all samples
        mask_realizations_per_sample=1,
        patch_size=1024,
        mask_ratio=MASK_RATIO,
        device='cuda:1',
        suffix=SUFFIX,
